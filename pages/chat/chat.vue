<template>
	<view class="chat-container">
		<scroll-view id="chatContainer" class="chat-content" :scroll-top="scroll.curHeight" scroll-y="true" scroll-with-animation="true">
			<view id="chatContent" v-for="(message,index) in messageList" :key="index">
				<view v-if="message.role==='user'" class="message-right">
					<text class="content-right">{{message.content}}</text>
				...
				<view v-if="message.role!=='user'" class="message-left">
					...
					<text class="content-left">{{message.content}}</text>
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		methods: {
			...
			sendMessage(message) {
				...
				this.messageList.push(this.pendingMsg) // 插入提示语
				this.scrollToBottom()
				uni.request({
					url: 'http://localhost:8590/chat-gpt/v1/chat/completions',
					method: 'POST',
					data: this.chatBody,
					success: (res) => {
						...
						if (lastMsg.role === 'assistant' && lastMsg.content.indexOf('我正在努力思考中') != -1) {
							this.messageList.pop() // 删除提示信息
						}
